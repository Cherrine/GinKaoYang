...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen(this.currentuser, {Key? key}) : super(key: key);
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  User get currentuser => widget.currentuser; // List to hold recent orders

  @override
  void initState() {
    super.initState();
    popularItems = menu_items
  ...
  Widget _buildOrderHistory(User user) {
    return OrderHistoryScreen(user : user); // Use a regular scaffold for order history
  }
          ...
          child: Builder(
            builder: (context) {
              switch (selectedRoute) {
                ...
                case "order_history":
                  return _buildOrderHistory(currentuser); // Updated OrderHistoryScreen
                default:
                  return const Center(
                    child: Text(
                      'Select a Menu Item',
      ...
      drawer: Sidebar(
        onNavigate: (routeName) {
          ...
          setState(() {
            ...
            appBarTitle =
                routeName == "home" ? "Home" : "Order History"; // Update title
          });
        },
      ),
